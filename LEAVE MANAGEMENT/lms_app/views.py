...
def home(request):
    if request.user.is_authenticated:
        ...
        try:
            ...
            # Check if the user's role allows access to leave links
            if user_role in ['HR', 'Team lead']:
                show_leave_link = "no"
        except Profile.DoesNotExist:
            # Handle case where user does not have a related Profile object
            # You can choose to handle this case differently based on your requirements
            user_profile = None
            user_role = None

        return render(request, 'index.html', context={'Uprofile': user_profile, 'name': user.username, 'roles': roles, 'current_user_role': user_role, 'show_leave': show_leave_link})
    else:
...
def leave_approval(request):
    ...
    return render(request, 'leave_approval.html', {'pending_requests': pending_requests})



def hod_dashboard(request):
    if request.user.userprofile.role == 'HOD':  # Check if the user is HOD
        pending_requests = LeaveRequest.objects.filter(status='pending')
        if request.method == 'POST':
            request_id = request.POST.get('request_id')
            if request_id:
        ...
        return render(request, 'hod_dashboard.html', {'pending_requests': pending_requests})
    else:
        return redirect('home')
